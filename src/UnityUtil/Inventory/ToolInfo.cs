namespace UnityEngine.Inventory {
    ...
    public class ToolInfo : ScriptableObject
    {
        ...
        [Tooltip(
            $"Once a (semi-)automatic {nameof(UnityEngine.Inventory.Tool)} starts being used, it performs this many uses per second while " +
        ...
        [Tooltip(
            $"A semi-automatic {nameof(UnityEngine.Inventory.Tool)} performs this many uses once the {nameof(Tool.UseInput)} is started. " +
        ...
        [Tooltip(
            $"After the previous use, this many seconds must pass before the {nameof(UnityEngine.Inventory.Tool)} will " +
            $"respond to {nameof(Tool.UseInput)} again."
        )]
        public float RefactoryPeriod = 0f;

        [Tooltip(
            $"For (semi-)automatic {nameof(UnityEngine.Inventory.Tool)}s, if this value is true, then {nameof(ToolInfo.AutomaticUseRate)} will be ignored, " +
            $"and the {nameof(UnityEngine.Inventory.Tool)} will recharge before every use while {nameof(Tool.UseInput)} is maintained."
        )]
