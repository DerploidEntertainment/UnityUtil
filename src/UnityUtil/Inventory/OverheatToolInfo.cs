namespace UnityEngine.Inventory {
    ...
    [CreateAssetMenu(fileName = "overheat-tool", menuName = $"{nameof(UnityUtil)}/{nameof(UnityEngine.Inventory)}/{nameof(UnityEngine.Inventory.OverheatToolInfo)}")]
    public class OverheatToolInfo : ScriptableObject
    {
        ...
        [Tooltip(
            $"If true, then the {nameof(UnityEngine.Inventory.Tool)} cools by an absolute amount per second. " +
            $"If false, then the {nameof(UnityEngine.Inventory.Tool)} cools by a fraction of {nameof(OverheatToolInfo.MaxHeat)} per second."
        )]
        public bool AbsoluteHeat = true;

        [Tooltip(
            $"The {nameof(UnityEngine.Inventory.Tool)} will cool by this many units " +
