...
public class HoverForce : MonoBehaviour
{
    ...
    [Tooltip(
        $"If the ground beneath the {nameof(HoverForce.HoveringRigidbody)} makes an angle to the upward direction that is steeper than this angle, " +
        $"then the hover force will not be applied. This prevents the {nameof(HoverForce.HoveringRigidbody)} from 'climbing' steep walls."
    )]
    [Range(0f, 90f)]
    public float MaxAngleToSurface = 60f;

    [Tooltip(
        $"If true, then the hover force will only be applied while the {nameof(HoverForce.HoveringRigidbody)} is grounded " +
        $"(i.e., <= {nameof(HoverForce.HoverHeight)} units above a surface), causing it to fall at the normal rate. " +
    ...
    [Tooltip(
        $"The maximum mass of {nameof(HoverForce.HoveringRigidbody)} that this {nameof(UnityEngine.HoverForce)} can keep aloft at " +
        $"the {nameof(HoverForce.HoverHeight)}. If set to Infinity, then a Rigidbody of any mass can be kept aloft at the same {nameof(HoverHeight)}; " +
        "otherwise, Rigidbodies more massive than this value will sink to the ground. " +
        $"Note that the hover force will automatically scale down for lower {nameof(HoverForce.HoveringRigidbody)} masses."
    )]
    public float MaxHoverableMass = 1f;

    [Tooltip(
        "Only Colliders matching this layer mask will be repelled against by the hover force. " +
        $"That is, the {nameof(HoverForce.HoveringRigidbody)} will 'fall through' colliders that are not in this layer mask."
    )]
    public LayerMask GroundLayerMask;

    [Tooltip(
        "What axis should be considered upward? That is, " +
        $"along what axis will the hover force push the {nameof(HoverForce.HoveringRigidbody)} to keep it aloft?"
    )]
    ...
    [Tooltip(
        ...
        $"this {nameof(UnityEngine.HoverForce)} and the {nameof(HoverForce.HoveringRigidbody)} are part of the same in-game entity. " +
        "If false, then the hover force will be applied using the AddForce() method, which ignores the position of" +
        $"this {nameof(UnityEngine.HoverForce)} relative to the {nameof(HoverForce.HoveringRigidbody)}."
    )]
