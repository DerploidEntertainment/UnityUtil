...
internal class RootLogger<T> : BaseUnityUtilLogger<T>
{
    ...
    public void UnregisteringSceneServices(Scene scene) =>
        LogInformation(id: 2, nameof(UnregisteringSceneServices), $"Unregistering services from scene '{{{nameof(scene)}}}'...", scene.name);

    public void UnregisteredAllSceneServices(Scene scene, int count) =>
        LogInformation(id: 3, nameof(UnregisteredAllSceneServices), $"Successfully unregistered all {{{nameof(count)}}} services from scene '{{{nameof(scene)}}}'", count, scene.name);
    ...
    public void UnregisterMissingSceneService(Scene scene) =>
        LogWarning(id: 0, nameof(UnregisterMissingSceneService), $"Cannot unregister services from scene '{{{nameof(scene)}}}' as none have been registered. Are you trying to destroy multiple service collections from the same scene?", scene.name);
